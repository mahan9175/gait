  ...
  def train(self):
    ...
    for e in range(self._params['max_epochs']):
      ...
      if self._params['predict_activity']:
          ...
          eval_loss_value = eval_loss[0].item() if hasattr(eval_loss[0], 'item') else eval_loss[0]
          act_loss_value = eval_loss[1].item() if hasattr(eval_loss[1], 'item') else eval_loss[1]
          accuracy_value = eval_loss[2].item() if hasattr(eval_loss[2], 'item') else eval_loss[2]
          
          self._scalars['act_loss_eval'] = act_loss_value
          self._scalars['accuracy'] = accuracy_value
          act_log = '; act_eval_loss {:.4f}; accuracy {:.4f}'.format(act_loss_value, accuracy_value)
          eval_activity_loss = act_loss_value
          eval_accuracy = accuracy_value
      else:
          # When predict_activity is False
          eval_loss_value = eval_loss.item() if hasattr(eval_loss, 'item') else eval_loss
          act_log = ''

      self._scalars['eval_loss'] = eval_loss_value
      print("[INFO] ({}) Epoch {:04d}; eval_loss: {:.4f}; lr: {:.2e}".format(
          thisname, e, eval_loss_value, self._params['learning_rate']) + act_log)
      # print("[INFO] ({}) Epoch {:04d}; eval_loss: {:.4f}; lr: {:.2e}".format(
