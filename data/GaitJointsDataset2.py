...
# In the load_data method:
    train_data = pickle.load(open(os.path.join(self.data_dir, "EPG_train_" + str(self.fold) + ".pkl"), "rb"))
    test_data = pickle.load(open(os.path.join(self.data_dir, "EPG_test_" + str(self.fold) + ".pkl"), "rb"))
    if self._mode == 'train':
  ...
  def _get_item_train(self, idx):
    ...
    # CRITICAL FIX: Get actual coordinate dimension from data
    coord_dim = x.shape[2]  # Should be 2 for 2D data
    input_size = coord_dim * _NMAJOR_JOINTS
    pose_size = coord_dim * _NMAJOR_JOINTS
    ...
    # CRITICAL FIX: Reshape to (frames, features) based on actual coordinate dimension
    x = x.reshape(N, coord_dim * _NMAJOR_JOINTS)
